# Возвращаем 1080p (1440p) на Twitch

## Способы

### (Бес)платный VPN

Да, можно использовать VPN/Proxy. Для возвращения нормального качества достаточно проксировать `usher.ttvnw.net` и `gql.twitch.tv`

> [!WARNING]
> [Стоит помнить, что 1440p поддерживается не в каждой стране.](https://help.twitch.tv/s/article/stream-quality?language=en_US#FAQ)
...
> [!NOTE]
> Методы ниже связаны с DNS ([XBox DNS](https://xbox-dns.ru) & [GeoHide](https://dns.geohide.ru:8443) & [dns.malw.link](https://info.dns.malw.link) & [DNS mafioznik.xyz](https://freedom.mafioznik.xyz))
>
> **Может привести к показу рекламы!**
...
- [DNS mafioznik.xyz](https://freedom.mafioznik.xyz)
  - [Актуальные данные есть на сайте freedom.mafioznik.xyz](https://freedom.mafioznik.xyz)
  - Данные для подключения
    - DNS IPv4
      - `103.27.157.38`
      - `103.27.157.100`
...
- [GeoHide](https://dns.geohide.ru:8443)
  - [Актуальные данные есть на сайте dns.geohide.ru:8443](https://dns.geohide.ru:8443)
  - Данные для подключения
    - DNS IPv4
      - `95.182.120.241`
      - `185.87.51.182`
...
> На момент января 2026 dns.malw.link не работает стабильно
...
Если перенаправлять только `usher.ttvnw.net` и `gql.twitch.tv` на нужный IP, то можно не менять DNS глобально

> [!TIP]
> [XBox DNS](https://xbox-dns.ru) не поддерживает подобного и не планирует в ближайшем будущем. Но продвинутые пользователи могут попробовать DNS Forwarding
>
...
- IP для Hosts
  - [DNS mafioznik.xyz](https://freedom.mafioznik.xyz) - `103.27.157.38`
  - [GeoHide](https://dns.geohide.ru:8443) - `45.155.204.190` и/или `95.182.120.241`
  - [dns.malw.link](https://info.dns.malw.link/hosts) - `185.246.223.127`

#### ПК (Windows / Linux / MacOS)

[Найдите hosts файл](https://1cloud.ru/help/dns/file-hosts) и внесите туда следующие строки

```
103.27.157.38 usher.ttvnw.net
103.27.157.38 gql.twitch.tv
```
